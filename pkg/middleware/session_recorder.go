...
// SessionRecorderMiddleware 自动记录对话历史到 JSON 文件
type SessionRecorderMiddleware struct {
	*BaseMiddleware
	store            *SessionStore
...
// NewSessionRecorderMiddleware 创建会话记录中间件
func NewSessionRecorderMiddleware(store *SessionStore) *SessionRecorderMiddleware {
...
func (m *SessionRecorderMiddleware) GetSessionStore() *SessionStore {
	return m.store
}

// BeforeAgent 在 Agent 开始执行前初始化会话记录
func (m *SessionRecorderMiddleware) BeforeAgent(ctx context.Context, state *agent.State) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *SessionRecorderMiddleware) BeforeModel(ctx context.Context, req *llm.ModelRequest) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	for i := m.lastMessageCount; i < len(req.Messages); i++ {
		msg := req.Messages[i]
		if msg.Role == "user" {
			m.sessionRecord.Messages = append(m.sessionRecord.Messages, MessageLog{
				Role:      "user",
				Content:   msg.Content,
...
func (m *SessionRecorderMiddleware) AfterModel(ctx context.Context, resp *llm.ModelResponse, state *agent.State) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *SessionRecorderMiddleware) AfterTool(ctx context.Context, result *llm.ToolResult, state *agent.State) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *SessionRecorderMiddleware) persistSession(ctx context.Context) error {
...
func (m *SessionRecorderMiddleware) loadExistingSession(ctx context.Context) bool {
...
func (m *SessionRecorderMiddleware) SetResumeMode(sessionID string) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *SessionRecorderMiddleware) ClearSession() {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *SessionRecorderMiddleware) LoadSessionMessages(ctx context.Context) ([]llm.Message, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	...
	for _, msg := range m.sessionRecord.Messages {
		...
		// 恢复工具调用上下文：为每个有 Output 的 ToolCall 生成对应的 user 消息
		if msg.Role == "assistant" {
			for _, tc := range msg.ToolCalls {
				if tc.Output != "" {
					messages = append(messages, llm.Message{
						Role:    llm.RoleUser,
						Content: fmt.Sprintf("Tool %s result: %s", tc.Name, tc.Output),
					})
				}
			}
		}
